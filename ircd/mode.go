...
func newModeSpec(ch rune, typ modeType, name string) *ModeSpec {
	var prefix string
	if typ == StatusMode {
		if idx := strings.IndexRune(statusMode, ch); idx >= 0 {
			prefix = statusPrefix
		}
	}
	return &ModeSpec{
...
func (am ActiveModes) Get(ch rune) (isset bool, args []string) {
	var m Mode
...
func (am ActiveModes) Match(ch rune, ref string) bool {
	var m Mode
	var isset bool
	if m, isset = am[ch]; !isset {
		return false
...
func (am ActiveModes) Contains(ch rune, ref string) bool {
	var m Mode
	var isset bool
	if m, isset = am[ch]; !isset {
		return false
